...
def postprocess(outfile, tmp, classes, op_properties, dp_properties, inverse_map, dsub_pairs,
                 source_graph=None, dprops_skipped=None):
    ...
    # ---------------------------------------------------------
    # RECOVER DATA PROPERTIES KONCLUDE COULD NOT ANSWER AT ALL
    # ---------------------------------------------------------
    # A dprops query line that still fails after the per-line retry in
    # run_one_job() means Konclude cannot answer that query, full stop
    # (e.g. it segfaults on it every time). Rather than silently losing
    # that property's values, read them as asserted directly from the
    # source ontology via rdflib -- same trade-off already accepted for
    # dsub_pairs above: we lose any reasoner-only inferred values for
    # that one property, but we no longer lose the asserted ones.
    if source_graph is not None and dprops_skipped:
        for q, code_i in dprops_skipped:
            m = re.search(r'IRI\("([^"]+)"\)', q)
            if not m:
                continue
            dp_iri = m.group(1)
            recovered = 0
            for subj, val in source_graph.subject_objects(URIRef(dp_iri)):
                if isinstance(subj, URIRef) and isinstance(val, Literal):
                    g.add((subj, URIRef(dp_iri), val))
                    data_assertions.append((subj, dp_iri, val))
                    recovered += 1
            print(f"[INFO] dprops: recovered {recovered} asserted (non-reasoned) "
                  f"triple(s) for {dp_iri} directly from source ontology "
                  f"(Konclude could not answer this query, exit {code_i}).")
...
def koncludix(binary, input_file, output_file, work_dir):
    ...
    postprocess(
        ...
        dprops_skipped=skipped_by_job.get("dprops", [])
    )
