...
namespace Konclude {
	...
	namespace Reasoner {
		...
		namespace Query {
			...
			void CWriteQueryFileOWL2FunctionalSerializer::writeObjectPropertyAssertion(const QString& subjectName, bool subjectAnonymous, const QString& propertyName, const QString& objectName, bool objectAnonymous) {
				QString subjectToken = subjectAnonymous ? subjectName : QString("<%1>").arg(subjectName);
				QString objectToken = objectAnonymous ? objectName : QString("<%1>").arg(objectName);
				QString writeString = QString("ObjectPropertyAssertion(<%1> %2 %3)\n").arg(propertyName, subjectToken, objectToken);
				mCurrentOutputFile->write(writeString.toUtf8());
			}


			void CWriteQueryFileOWL2FunctionalSerializer::writeDataPropertyAssertion(const QString& subjectName, bool subjectAnonymous, const QString& propertyName, const QString& lexicalValue, const QString& datatypeIRI) {
				QString subjectToken = subjectAnonymous ? subjectName : QString("<%1>").arg(subjectName);
				QString escapedValue = lexicalValue;
				escapedValue.replace(QLatin1Char('\\'), QLatin1String("\\\\"));
				escapedValue.replace(QLatin1Char('"'), QLatin1String("\\\""));
				QString writeString = QString("DataPropertyAssertion(<%1> %2 \"%3\"^^<%4>)\n").arg(propertyName, subjectToken, escapedValue, datatypeIRI);
				mCurrentOutputFile->write(writeString.toUtf8());
			}



			void CWriteQueryFileOWL2FunctionalSerializer::writeIndividualEquivalenceRelations(const QStringList& individualNameList, const QList<bool>& individualAnonymousList) {
				QStringList tokenList;
				for (int i = 0; i < individualNameList.size(); ++i) {
					bool anonymous = i < individualAnonymousList.size() && individualAnonymousList.at(i);
					tokenList.append(anonymous ? individualNameList.at(i) : QString("<%1>").arg(individualNameList.at(i)));
				}
				QString writeString = QString("SameIndividual(%1)\n").arg(tokenList.join(" "));
				mCurrentOutputFile->write(writeString.toUtf8());
			}
