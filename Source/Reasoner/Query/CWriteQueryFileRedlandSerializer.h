		...
		namespace Query {
			...
			class CWriteQueryFileRedlandSerializer : public CWriteQuerySerializer {
				...
				protected:
					...
					// Every write*() method funnels through these two -- the
					// actual triple realization (direct text line vs. librdf
					// model) is decided once, here, based on mDirectTextMode.
					void addResourceTriple(const QString& subjectName, bool subjectAnonymous, const QString& predicateIRI, const QString& objectName, bool objectAnonymous);
					void addLiteralTriple(const QString& subjectName, bool subjectAnonymous, const QString& predicateIRI, const QString& lexicalValue, const QString& datatypeIRI);

					// Direct-text fast path (Turtle/N-Triples): full IRIs only,
					// no @prefix abbreviation -- see mDirectTextMode comment.
					QString formatResource(const QString& name, bool anonymous);
					QString formatLiteral(const QString& lexicalValue, const QString& datatypeIRI);
					QString escapeLiteralValue(const QString& value);
					void writeTripleLine(const QString& subjectText, const QString& predicateText, const QString& objectText);
				...
				protected:
					...
					// librdf's "memory" storage does an O(n) scan per
					// librdf_model_add() call (confirmed by direct benchmark:
					// per-triple insert cost grows linearly with the triple count
					// already added, i.e. O(n^2) total) -- fine for the small
					// hierarchy/declaration triples, but writing tens of thousands
					// of individual assertions through it turns minutes into a
					// real problem. Turtle and N-Triples don't need librdf's
					// model/serializer machinery at all -- they're written
					// directly as text here, exactly like the OWL2-XML/Functional
					// serializers already do. RDF/XML is intricate enough
					// (striping, xml:lang, escaping) that it still goes through
					// librdf's model+serializer below, so it keeps the O(n^2)
					// cost at scale; nothing exercises that path in practice here.
					bool mDirectTextMode;
					QFile* mOutputFile;
