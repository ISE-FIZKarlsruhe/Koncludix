				...
				bool COWLlinkProcessor::processCustomsEvents(QEvent::Type type, CCustomEvent *event) {
					if (!CThread::processCustomsEvents(type,event)) {
						if (type == EVENTREALIZECOMMAND) {
							...
							} else if (dynamic_cast<CForceRoleRealizationQueryCommand *>(command)) {
								CCommandRecordRouter commandRecordRouter(command,this);
								CStartProcessCommandRecord::makeRecord(&commandRecordRouter);
								CForceRoleRealizationQueryCommand* frrqC = (CForceRoleRealizationQueryCommand*)command;

								CBuildQueryCommand *buildQC = frrqC->getBuildQueryCommand();
								if (!buildQC) {
									buildQC = new CBuildQueryCommand(0,frrqC);
									frrqC->setBuildQueryCommand(buildQC);
								}

								CCalculateQueryCommand *calcQC = frrqC->getCalculateQueryCommand();
								if (!calcQC) {
									calcQC = new CCalculateQueryCommand(buildQC,frrqC);
									frrqC->setCalculateQueryCommand(calcQC);
								}

								CGetCurrentKnowledgeBaseRevisionCommand *getCKBRC = new CGetCurrentKnowledgeBaseRevisionCommand(frrqC->getKnowledgeBaseName());
								buildQC->makeToSubCommand(getCKBRC);

								CConstructForceRoleRealizationQueryCommand *pOWLQC = new CConstructForceRoleRealizationQueryCommand(getCKBRC);
								buildQC->makeToSubCommand(pOWLQC);
								buildQC->setQueryCommandProvider(pOWLQC);
							...
							} else if (dynamic_cast<CForceSameIndividualsRealizationQueryCommand *>(command)) {
								CCommandRecordRouter commandRecordRouter(command,this);
								CStartProcessCommandRecord::makeRecord(&commandRecordRouter);
								CForceSameIndividualsRealizationQueryCommand* fsirqC = (CForceSameIndividualsRealizationQueryCommand*)command;

								CBuildQueryCommand *buildQC = fsirqC->getBuildQueryCommand();
								if (!buildQC) {
									buildQC = new CBuildQueryCommand(0,fsirqC);
									fsirqC->setBuildQueryCommand(buildQC);
								}

								CCalculateQueryCommand *calcQC = fsirqC->getCalculateQueryCommand();
								if (!calcQC) {
									calcQC = new CCalculateQueryCommand(buildQC,fsirqC);
									fsirqC->setCalculateQueryCommand(calcQC);
								}

								CGetCurrentKnowledgeBaseRevisionCommand *getCKBRC = new CGetCurrentKnowledgeBaseRevisionCommand(fsirqC->getKnowledgeBaseName());
								buildQC->makeToSubCommand(getCKBRC);

								CConstructForceSameIndividualsRealizationQueryCommand *pOWLQC = new CConstructForceSameIndividualsRealizationQueryCommand(getCKBRC);
								buildQC->makeToSubCommand(pOWLQC);
								buildQC->setQueryCommandProvider(pOWLQC);
								...
								CFinishProcessCommandRecord::makeRecord(&commandRecordRouter);
										...
										if (ontRev) {
											...
											if (queryType == CWriteQuery::WRITESUBCLASSHIERARCHY) {
												...
												cWCQC->setQuery(query);
							} else if (queryType == CWriteQuery::WRITEMATERIALIZEDINDIVIDUALASSERTIONS) {
								CWriteSerializerMaterializedIndividualAssertionsQuery *query = new CWriteSerializerMaterializedIndividualAssertionsQuery(ontRev->getOntology(),ontConfig,serializer,entityRestrictionString);
								cWCQC->setQuery(query);
											}
						...
						} else if (dynamic_cast<CConstructForceRoleRealizationQueryCommand *>(command)) {
							CCommandRecordRouter commandRecordRouter(command,this);
							CStartProcessCommandRecord::makeRecord(&commandRecordRouter);

							CConstructForceRoleRealizationQueryCommand *cFRRQC = (CConstructForceRoleRealizationQueryCommand*)command;
							COntologyRevision *ontRev = cFRRQC->getOntologyRevision();
							if (ontRev) {
								COntologyConfigurationExtension *ontConfig = ontRev->getOntologyConfiguration();
								CForceRoleRealizationQuery *query = new CForceRoleRealizationQuery(ontRev->getOntology(),ontConfig);
								cFRRQC->setQuery(query);
							} else {
								CUnspecifiedMessageErrorRecord::makeRecord("Knowledge base revision not available.",&commandRecordRouter);
							}
					...
					} else if (dynamic_cast<CConstructForceSameIndividualsRealizationQueryCommand *>(command)) {
						CCommandRecordRouter commandRecordRouter(command,this);
						CStartProcessCommandRecord::makeRecord(&commandRecordRouter);

						CConstructForceSameIndividualsRealizationQueryCommand *cFSIRQC = (CConstructForceSameIndividualsRealizationQueryCommand*)command;
						COntologyRevision *ontRevSame = cFSIRQC->getOntologyRevision();
						if (ontRevSame) {
							COntologyConfigurationExtension *ontConfigSame = ontRevSame->getOntologyConfiguration();
							CForceSameIndividualsRealizationQuery *query = new CForceSameIndividualsRealizationQuery(ontRevSame->getOntology(),ontConfigSame);
							cFSIRQC->setQuery(query);
						} else {
							CUnspecifiedMessageErrorRecord::makeRecord("Knowledge base revision not available.",&commandRecordRouter);
						}
