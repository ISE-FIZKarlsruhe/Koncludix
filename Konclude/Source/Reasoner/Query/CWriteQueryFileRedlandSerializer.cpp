		...
		namespace Query {
			...
			void CWriteQueryFileRedlandSerializer::addTypeTriple(const QString& subjectName, bool subjectAnonymous, const QString& classIRI) {
				addTriple(createResourceNode(subjectName, subjectAnonymous),
						  librdf_new_node_from_uri_string(mWorld, (const unsigned char*)RDF_TYPE),
						  librdf_new_node_from_uri_string(mWorld, (const unsigned char*)classIRI.toUtf8().constData()));
			}
			...
			void CWriteQueryFileRedlandSerializer::addPairwiseChainRelation(const QStringList& nameList, const QString& relationIRI) {
				...
				for (int i = 0; i + 1 < nameList.size(); ++i) {
					addTriple(librdf_new_node_from_uri_string(mWorld, (const unsigned char*)nameList.at(i).toUtf8().constData()),
							  librdf_new_node_from_uri_string(mWorld, (const unsigned char*)relationIRI.toUtf8().constData()),
							  librdf_new_node_from_uri_string(mWorld, (const unsigned char*)nameList.at(i+1).toUtf8().constData()));
				}
			}
			...
			void CWriteQueryFileRedlandSerializer::writeSubObjectPropertyRelation(const QString& subPropertyName, const QString& superPropertyName) {
				addTriple(librdf_new_node_from_uri_string(mWorld, (const unsigned char*)subPropertyName.toUtf8().constData()),
						  librdf_new_node_from_uri_string(mWorld, (const unsigned char*)RDFS_SUBPROPERTYOF),
						  librdf_new_node_from_uri_string(mWorld, (const unsigned char*)superPropertyName.toUtf8().constData()));
			}
			...
			void CWriteQueryFileRedlandSerializer::writeSubClassRelation(const QString& subClassName, const QString& superClassName) {
				addTriple(librdf_new_node_from_uri_string(mWorld, (const unsigned char*)subClassName.toUtf8().constData()),
						  librdf_new_node_from_uri_string(mWorld, (const unsigned char*)RDFS_SUBCLASSOF),
						  librdf_new_node_from_uri_string(mWorld, (const unsigned char*)superClassName.toUtf8().constData()));
			}
			...
			void CWriteQueryFileRedlandSerializer::writeObjectPropertyAssertion(const QString& subjectName, bool subjectAnonymous, const QString& propertyName, const QString& objectName, bool objectAnonymous) {
				addTriple(createResourceNode(subjectName, subjectAnonymous),
						  librdf_new_node_from_uri_string(mWorld, (const unsigned char*)propertyName.toUtf8().constData()),
			...
			void CWriteQueryFileRedlandSerializer::writeIndividualEquivalenceRelations(const QStringList& individualNameList, const QList<bool>& individualAnonymousList) {
				for (int i = 0; i + 1 < individualNameList.size(); ++i) {
					...
					addTriple(createResourceNode(individualNameList.at(i), anon1),
							  librdf_new_node_from_uri_string(mWorld, (const unsigned char*)OWL_SAMEAS),
							  createResourceNode(individualNameList.at(i+1), anon2));
				}
			}
